...
export class WhatsAppClient {
        ...
        this.init(false)
            .then(async () => {
                this.client
                    ...
                    .catch((e) =>
                        console.log('Error getting the version', e.message)
                    )
                console.log(
                    'authenticated after init server',
                    this.wpClient.alias
                )
                this.starting = false
            })
    ...
    onReady = (): void => {
        ...
        this.chatBot.sync()
        WpNotificationRepository.onServiceAssigned(
            this.wpClient.id,
            this.serviceAssigned
        )
        WpNotificationRepository.onDriverArrived(
            this.wpClient.id,
            this.driverArrived
        )
        WpNotificationRepository.onNewService(
            this.wpClient.id,
            this.onNewService
        )
        WpNotificationRepository.onServiceCanceled(
            this.wpClient.id,
            this.serviceCanceled
        )
        WpNotificationRepository.onServiceTerminated(
            this.wpClient.id,
            this.serviceTerminated
        )
    ...
    onQR = (qr: string): void => {
        if (this.socket)
            this.socket.to(this.wpClient.id).emit(WpEvents.QR_RECEIVED, qr)
    ...
    onDisconnected = async (reason: string | WpStates): Promise<void> => {
        ...
        if (this.socket)
            this.socket.to(this.wpClient.id).emit(WpEvents.DISCONNECTED, reason)
    ...
    onAuthFailure = (message: string): void => {
        if (this.socket)
            this.socket
                .to(this.wpClient.id)
                .emit(WpEvents.AUTHENTICATION_FAILURE, message)
        console.log(
    ...
    onLoadingScreen = (percent: string, message: string): void => {
        ...
        if (this.socket)
            this.socket
                .to(this.wpClient.id)
                .emit(WpEvents.LOADING_SCREEN, loading)
        console.log(
    ...
    onStateChanged = (waState: WpStates): void => {
        if (this.socket)
            this.socket.to(this.wpClient.id).emit(EmitEvents.GET_STATE, waState)
        ...
        this.client
            ...
            .then((state) => {
                if (this.socket)
                    this.socket
                        .to(this.wpClient.id)
                        .emit(EmitEvents.GET_STATE, state)
            })
            .catch((e) => {
                console.log(
                    EmitEvents.GET_STATE,
                    this.wpClient.alias,
                    e.message
                )
                if (this.socket)
                    this.socket
                        .to(this.wpClient.id)
                        .emit(EmitEvents.GET_STATE, WpStates.UNPAIRED)
            })
    }
    ...
    serviceAssigned = async (snapshot: DataSnapshot): Promise<void> => {
        ...
        if (
            ...
            notification.wp_client_id == this.wpClient.id
        ) {
            ...
            if (msg.enabled) {
                await this.sendMessage(
                    ...
                    WpNotificationRepository.deleteNotification(
                        'assigned',
                        snapshot.key ?? ''
                    )
                })
    ...
    cancelTimeout = (serviceId: string, clientId: string): void => {
        setTimeout(async () => {
            await ServiceRepository.findServiceStatusById(serviceId).then(
                async (status) => {
                    if (status === Service.STATUS_PENDING) {
                        const msg = Messages.getSingleMessage(
    ...
    logout = async (): Promise<void> => {
        await this.client
            ...
            .then(async () => {
                ...
                if (this.socket)
                    this.socket.to(this.wpClient.id).emit('destroy')
            })
            .catch((e) => {
                ...
                if (this.socket)
                    this.socket
                        .to(this.wpClient.id)
                        .emit(EmitEvents.FAILURE, e.message)
                Sentry.captureException(e)
                exit(1)
            })
    }
    ...
    serviceChanged = async (snapshot: DataSnapshot): Promise<void> => {
        if (!this.wpClient.chatBot && !this.wpClient.assistant)
        ...
        switch (service.status) {
            case Service.STATUS_IN_PROGRESS:
                ...
                if (!service.metadata) {
                    ...
                    service.metadata.arrived_at > 0 &&
                    !service.metadata.start_trip_at
                ) {
                    if (!session.notifications.arrived) {
                        await session.setNotification(NotificationType.arrived)
                        msg = Messages.getSingleMessage(
