...
export default class Session implements SessionInterface {
        ...
        await SessionRepository.addMsg(this.id, wpMessage)
            .then(async (key) => {
                if (wpMessage.type === MessageTypes.LOCATION) {
                    ...
                    const unprocessedMessagesArray = Array.from(
                        this.getUnprocessedMessages().values()
                    )
                    await this.processMessage(
                        wpMessage,
                        unprocessedMessagesArray
                    )
                } else {
                    this.messages.set(key, wpMessage)
    ...
    async processUnprocessedMessages(): Promise<void> {
        ...
        if (
            ...
            this.processorTimeout = setTimeout(() => {
                ...
                const text = unprocessedMessagesArray
                    .map((msg) => msg.msg)
                    .join(' ')
    ...
    async processMessage(
        ...
        await response
            ...
            .then(async () => {
                await SessionRepository.setProcessedMsgs(
                    this.id,
                    unprocessedMessages
                ).then(() => {
            ...
            .catch(async (e) => {
                ...
                const msg = getSingleMessage(
                    MessagesEnum.ERROR_WHILE_PROCESSING
                )
                if (msg.enabled) {
