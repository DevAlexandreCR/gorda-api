...
type BillingEmailTemplateParams = {
...
export function buildBillingEmailHtml({
  ...
  extras,
  totalCop,
}: BillingEmailTemplateParams): string {
  const summaryRows: Array<[string, string]> = [
    ['Periodo facturado', `${summary.startDateLabel} al ${summary.endDateLabel}`],
...
<body style="margin:0;padding:24px;background:#f4f6fb;font-family:Arial,sans-serif;color:#344767 !important;">
  <div style="max-width:680px;margin:0 auto;background:#ffffff;border:1px solid #e6eaf2;border-radius:18px;overflow:hidden;box-shadow:0 16px 36px rgba(15,23,42,0.08);color:#344767 !important;">
    ...
    <div style="padding:0 30px 30px;">
      ...
      ${buildMatrixTable(['Línea', 'Sesiones', 'Entrantes', 'Salientes'], activityRows)}
...
function buildKeyValueTable(rows: Array<[string, string]>): string {
  const body = rows
    .map(
      ([label, value]) => `<tr>
      <td style="${CELL_LABEL_STYLE}">${escapeHtml(label)}</td>
      <td style="${CELL_VALUE_STYLE}">${escapeHtml(value)}</td>
    </tr>`
    )
    .join('')
